...
def ocr_bbox (image,bbox) : 
    # separate bounding box coordinates from the original image
    x1 = int(bbox[0][0][0][1])
    y1 = int(bbox[0][0][0][0])
    x2 = int(bbox[0][0][0][3])
    y2 = int(bbox[0][0][0][2])
    ...
    # get the subimage that makes up the bounded region (ROI) and take an additional 5 pixels on each side
    croped = image[y1-3:y2+3 , x1-3:x2+3]
